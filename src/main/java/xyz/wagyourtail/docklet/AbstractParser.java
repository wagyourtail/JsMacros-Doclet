...
public abstract class AbstractParser {
    ...
    private static String transformType(Type type) {
        if (type.asTypeVariable() != null) {
            ...
            return "any";
        } else if (JsMacrosDocklet.events.stream().flatMap(e -> Stream.of(e.clazz.qualifiedTypeName())).anyMatch(e -> e.equals(type.qualifiedTypeName()))) {
            return "Events." + JsMacrosDocklet.events.stream().filter(e -> e.clazz.qualifiedTypeName().equals(type.qualifiedTypeName())).findFirst().get().name;
    ...
    public static String genCommentTypeScript(Tag[] inline, boolean isStatic, int tabLevel) {
        ...
        if (isStatic) comment += inline.length > 0 ? "\n\nstatic" : "static";
        String delim = "";
        for (int i = 0; i < tabLevel; ++i) {
            delim += "\t";
        }
        comment = insertEachLine(comment, delim + " *");
        return "\n" + delim + "/**\n" + comment + "\n" + delim + " */" + "\n";
    }
}
